...
class FOPMixer(nn.Module):
    ...
    def forward(self, agent_qs, states, actions=None, vs=None):
        ...
        std = th.exp(0.5 * lambda_weight2)
        eps = th.randn_like(std)
    
        # Reparameterization trick
        sampled_latent = lambda_weight + eps * std

        sigma_squared = th.exp(lambda_weight2)
        entropy = 0.5 * (1 + th.log(2 * th.pi * sigma_squared))
        entropy_weights = F.softmax(entropy, dim=-1)

        adv_tot = th.sum(adv_q * sampled_latent, dim=1).reshape(bs, -1, 1)
        v_tot = th.sum(agent_qs + v, dim=-1).reshape(bs, -1, 1)

        return adv_tot + v_tot, entropy_weights 
